...
router.put('/products/:idproducts_local',function(req,res){
    ...
    mysqlConnection.query("UPDATE products_local SET stock=?, ennabled=? ,price_buy=?, price_sell=?, price_wholesale=?, category=?,local=? WHERE idproducts_local=?",[stock,ennabled,price_buy,price_sell,price_wholesale,category,local,idproducts_local],(err,row,field)=>{
     if(err){
         console.log(err);
  
     }else{
         res.json({"status":"Products_local update"});
     }
     })
});
